...
export default function LeadTable({ leads, onEdit, onDelete, selected = [], onSelect, onSelectAll }) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[860px] text-left text-sm">
          <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              {onSelect && <th className="px-4 py-3"><input type="checkbox" checked={leads.length > 0 && leads.every((lead) => selected.includes(lead.id))} onChange={onSelectAll} /></th>}
          ...
          <tbody className="divide-y divide-slate-100">
            {leads.map((lead) => (
              <tr key={lead.id} className="hover:bg-slate-50/80">
                {onSelect && <td className="px-4 py-3"><input type="checkbox" checked={selected.includes(lead.id)} onChange={() => onSelect(lead.id)} /></td>}
                ...
                <td className="px-4 py-3 text-slate-600">
                  {lead.serviceOpportunities?.[0]
                    ? `$${lead.serviceOpportunities[0].estimatedMinValue.toLocaleString()} - $${lead.serviceOpportunities[0].estimatedMaxValue.toLocaleString()}`
