...
function senderFromAccount(account) {
  const emailName = account?.email?.split("@")[0];

  return {
    name: account?.name || account?.username || emailName || "Sender",
    email: account?.email || "",
    title: account?.title || "Founder",
    company: account?.company || "Ocia Studio",
  };
}

function formatEmailBody(message, lead, sender, assistantMode = false) {
  const raw = messageWithoutSubject(message || "")
    .replace(/\s*thanks,?\s*[\s\S]*$/i, "")
    .trim();

  const firstName =
    lead?.ownerName || lead?.contactName || lead?.firstName || "there";

  const signature = assistantMode
    ? [sender.name]
...
function senderFromAccount(account) {
  ...
  return {
    name: account?.name || account?.displayName || account?.username || emailName || "Sender",
    email: account?.email || "",
    title: account?.title || "Founder",
    company: account?.company || "Ocia Studio"
  };
...
function cleanAiBody(message) {
  return messageWithoutSubject(message || "")
    .replace(/^\s*(hi|hello|dear)\s+[^,\n]+,?\s*/i, "")
    .replace(/\s*(thanks|thank you|regards|best),?\s*[\s\S]*$/i, "")
    .trim()
    .split(/\n+/)
    .map((line) => line.trim())
    .filter(Boolean)
...
function buildEmailBody(message, lead, sender, assistantMode = false) {
  const firstName =
    lead?.ownerName ||
    lead?.contactName ||
    lead?.firstName ||
...
export default function EmailsPage() {
  ...
  async function loadLeads() {
    ...
    try {
      ...
      push(
        error.response?.data?.message || "Could not load email leads",
        "error",
      );
    } finally {
      setLoading(false);
    ...
    api
      ...
      .then(({ data }) => {
        const lead = {
          ...
          recommendedService:
            data.serviceOpportunities?.find((item) => item.recommended)
              ?.service ||
            data.serviceOpportunities?.[0]?.service ||
            null,
        };
        setSelected([lead.id]);
      ...
      .catch((error) =>
        push(
          error.response?.data?.message || "Could not open lead email preview",
          "error",
        ),
      );
  }, []);
  ...
  async function generateSelected() {
    ...
    setProcessing(true);
    try {
      const { data } = await api.post("/emails/generate", {
        leadIds: selected,
      });
  ...
  async function startApproval() {
    if (!selected.length) return push("Select leads first", "error");
    if (!gmailReady)
      return push("Connect ociastudios@gmail.com before sending", "error");
    const queue = selectedLeads.length
      ? selectedLeads
      : leads.filter((lead) => selected.includes(lead.id));
  ...
  async function loadPreview(lead) {
    ...
    try {
      ...
      setPreview({
        ...
        issue:
          draft?.opener || "No reliable evidence-backed observation available.",
        service: lead.recommendedService?.name || "Website redesign",
      });
    } catch (error) {
      push(
        error.response?.data?.message || "Could not generate preview",
        "error",
      );
    } finally {
      setProcessing(false);
  ...
  async function saveDraft() {
    ...
    await api.put(`/outreach/${preview.draftId}`, {
      subject: preview.subject,
      fullMessage: preview.body,
      status: "SAVED",
    });
    push("Draft saved");
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-slate-500">
            Search leads, generate emails, preview and edit each message, then
            send manually.
          </p>
        </div>
        <Link
          to="/settings/email"
          className="inline-flex items-center justify-center rounded-lg bg-white px-3.5 py-2 text-sm font-semibold text-slate-900 ring-1 ring-slate-200 hover:bg-slate-50"
        >
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-4 xl:grid-cols-6">
          ...
          <Select
            value={filters.recommendedServiceId}
            onChange={(e) =>
              setFilters({ ...filters, recommendedServiceId: e.target.value })
            }
          >
          ...
          <Select
            ...
            onChange={(e) =>
              setFilters({ ...filters, contactState: e.target.value })
            }
          >
          ...
          <Select
            value={filters.hasEmailOnly}
            onChange={(e) =>
              setFilters({ ...filters, hasEmailOnly: e.target.value })
            }
          >
          ...
          <Select
            value={filters.missingBooking}
            onChange={(e) =>
              setFilters({ ...filters, missingBooking: e.target.value })
            }
          >
          ...
          <Select
            value={filters.missingAnalytics}
            onChange={(e) =>
              setFilters({ ...filters, missingAnalytics: e.target.value })
            }
          >
          ...
          <Select
            value={filters.withoutMetaPixel}
            onChange={(e) =>
              setFilters({ ...filters, withoutMetaPixel: e.target.value })
            }
          >
        ...
        <div className="sticky top-20 z-10 flex flex-col gap-3 rounded-3xl border border-slate-200 bg-slate-950 p-4 text-white shadow-glow md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              disabled={processing}
            >
              <MailPlus size={16} /> Generate emails
            </Button>
            <Button
              variant="secondary"
              ...
              disabled={processing}
            >
              <CheckCircle2 size={16} /> Generate + send with approval
            </Button>
            <Button
              variant="ghost"
        ...
        <div className="overflow-auto">
            ...
            <tbody>
              {leads.map((lead) => (
                <tr key={lead.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">
                    {lead.industryRef?.name || lead.industry || "-"}
                  </td>
                  <td className="px-4 py-3">{domain(lead.website)}</td>
                  <td className="px-4 py-3">
                    <Badge className={priorities[lead.priority]?.className}>
                      {lead.score}/10
                    </Badge>
                  </td>
                  <td className="px-4 py-3">{money(lead.estimatedValue)}</td>
                  <td className="px-4 py-3">
                    {lead.recommendedService?.name || "-"}
                  </td>
                  <td className="px-4 py-3">
                    {lead.contactEmail || (
          ...
          {!leads.length && (
            <p className="p-8 text-center text-sm text-slate-500">
              No qualified email leads match these filters.
            </p>
          )}
        </div>
      </section>
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-3xl bg-white p-6 shadow-glow">
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Lead {approvalIndex + 1} of {approvalQueue.length || 1}
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <label>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setPreview({ ...preview, toEmail: e.target.value })
                  }
              ...
              <label>
                ...
                <Textarea
                  className="min-h-96 whitespace-pre-wrap font-mono text-sm leading-6"
                  value={preview.body}
                  onChange={(e) =>
              ...
              <label>
                <span className="mb-1.5 block text-sm font-medium">
                  Test email before sending to lead
                </span>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={processing || !gmailReady}
                  >
                    Send test
                  </Button>
                </div>
            ...
            <div className="mt-5 flex flex-wrap justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-3">
                <Button variant="secondary" onClick={nextPreview}>
                  Skip
                </Button>
                <Button variant="secondary" onClick={nextPreview}>
                  Next
